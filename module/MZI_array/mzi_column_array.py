...
class MZIlayer_column(nn.Module):
    ...
    def forward(self, input):
        ...
        mid_input = input[:, :, :, 1:last_input_num]  # Shape: (batch_size, height, width, last_input_num-1)
        
        # Reshape for MZI processing
        mid_input_reshaped = mid_input.view(batch_size, height, width, self.num, 2)  
        
        # Process through n MZIs
        mzi_outputs = []
        for i, mzi in enumerate(self.MZI):
            # Get input pair for current MZI
            mzi_input = mid_input_reshaped[:, :, :, i, :]  # Shape: (batch_size, height, width, 2)
            
            # Prepare 4-port input for MZI
            full_input = torch.zeros((batch_size, height, width, 4), 
                                     dtype=torch.complex64, device=input.device)
            full_input[:, :, :, 0] = mzi_input[:, :, :, 0]  # First port
            full_input[:, :, :, 2] = mzi_input[:, :, :, 1]  # Third port
            
            # Process through MZI
            mzi_output = mzi(full_input.view(-1, 4)).view(batch_size, height, width, 4)
        ...
        mzi_outputs_tensor = torch.cat(mzi_outputs, dim=3)  # Shape: (batch_size, height, width, num*2)
    
        # Place processed middle section back into output tensor
        output[:, :, :, 1:last_input_num] = mzi_outputs_tensor
    
        return output  # Shape: (batch_size, height, width, num*2+2)
