...
data_o1 = data[data['output_port'] == 1]
...
def fit_mzi_parameters(voltage_data, power_data, global_p_max, global_p_min):
    ...
    try:
        ...
        # 返回完整参数 [a, b, p_max, p_min]
        return [popt[0], popt[1], global_p_max, global_p_min]

    except Exception as e:
        print(f"Fitting failed: {e}")
        # If fitting fails, use theoretical estimate
        return [a_guess, 0, global_p_max, global_p_min]
...
for i, mzi_num in enumerate(mzi_numbers):
    ...
    if not mzi_data.empty:
        ...
        # Fit parameters
        fitted_params = fit_mzi_parameters(voltage_points, power_points, global_p_max, global_p_min)
...
# Save the plot
plt.savefig(r'C:\Users\17958\AppData\Roaming\JetBrains\PyCharm2025.2\scratches\mzi_correction\in9\mzi_power_analysis_fitted.png', dpi=300, bbox_inches='tight')
