...
TARGET_MZI = 5  # Change this value to select MZI to analyze (0, 5, 10, 15, 20, 25, 30, 35, 40, 49)

# Read the data from the text file
file_path = r'C:\Users\17958\AppData\Roaming\JetBrains\PyCharm2025.2\scratches\mzi_correction\in9\MZI_array_output.txt'
...
mzi_numbers = [0, 5, 10, 15, 20, 25, 30, 35, 40, 49]

# Filter for output port o1
data_o1 = data[data['output_port'] == 0]

# Find global max/min power values across all selected MZIs
all_power_data = []
for mzi_num in mzi_numbers:
    mzi_data = data_o1[data_o1['input_mzi_number'] == mzi_num]
    if not mzi_data.empty:
        all_power_data.extend(mzi_data['output_power'].values)

global_p_max = np.max(all_power_data)
global_p_min = np.min(all_power_data)
print(f"Global power range: P_max = {global_p_max:.6f} W, P_min = {global_p_min:.6f} W")
...
def fit_mzi_parameters(voltage_data, power_data, global_p_max, global_p_min):
    """
    Fit MZI parameters based on phi = a * V^2 + b relationship
    Using global power max/min values as fixed parameters
    """
    # Simplified model, only fit phase parameters a and b
    def simplified_mzi_model(voltage, a, b):
        phase = a * voltage**2 + b
        power = global_p_min + (global_p_max - global_p_min) * np.cos(phase / 2)**2
        return power
    ...
    try:
        ...
        return [popt[0], popt[1], global_p_max, global_p_min], r_squared, rmse, pcov

    except Exception as e:
        print(f"Fitting failed: {e}")
        # If fitting fails, use theoretical estimate
        return [a_guess, 0, global_p_max, global_p_min], 0, float('inf'), None
...
print(f"Power range: {np.min(power_points):.6f}W to {np.max(power_points):.6f}W")

# Fit parameters
fitted_params, r_squared, rmse, pcov = fit_mzi_parameters(voltage_points, power_points, global_p_max, global_p_min)
...
# Save figure
output_filename = f'mzi_{TARGET_MZI}_detailed_analysis1.png'
