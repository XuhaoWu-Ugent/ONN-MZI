...
class PatchEmbedding(nn.Module):
    ...
    """
    def __init__(self, img_size=32, patch_size=4, in_channels=3, embed_dim=96): # Updated defaults
        super().__init__()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=32, patch_size=4, in_channels=3, num_classes=10,
                 embed_dim=96, depth=6, num_heads=4, mlp_ratio=4.0):
        super().__init__()
...
def get_dataloaders(cfg, distributed=False):
    """Create CIFAR-10 dataloaders with data augmentation"""
...
def save_checkpoint(model, optimizer, scheduler, scaler, epoch, best_acc, patience_counter, cfg, args, rank, is_best):
...
def load_checkpoint(model, optimizer, scheduler, scaler, args, rank):
    ...
    if is_main_process(rank):
        print(f"Resumed from epoch {checkpoint['epoch']}, best acc: {best_acc:.2f}%, patience_counter: {patience_counter}")
...
def main():
    ...
    # Load checkpoint
    start_epoch, best_acc, patience_counter = load_checkpoint(model, optimizer, scheduler, scaler, args, rank)
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        if acc > best_acc:
            ...
            if is_main:
                print(f"[Checkpoint] New best accuracy: {best_acc:.2f}%. Saving model...")
                save_checkpoint(model, optimizer, scheduler, scaler, epoch, best_acc, patience_counter, cfg, args, rank, is_best=True)
        else:
            patience_counter += 1
            if is_main:
                print(f"[EarlyStopping] Accuracy did not improve for {patience_counter}/{cfg.patience} epochs.")
            
        # Always save latest checkpoint
        save_checkpoint(model, optimizer, scheduler, scaler, epoch, best_acc, patience_counter, cfg, args, rank, is_best=False)

        if patience_counter >= cfg.patience:
            if is_main:
                print(f"\n[EarlyStopping] No improvement in accuracy for {cfg.patience} epochs. Stopping training.")
            break
